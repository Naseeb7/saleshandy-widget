...
export function RatingInput({ value, register, error }: RatingInputProps) {
  return (
    <fieldset>
        ...
        {ratings.map((rating) => (
          <label
            className={cn(
              ...
              value === rating && "border-primary bg-surface-muted text-primary",
            )}
            key={rating}
          >
            <input
            ...
            />
            <span aria-hidden="true" className="text-lg leading-none text-warning">
              ★
            </span>
            <span>{rating}</span>
          </label>
